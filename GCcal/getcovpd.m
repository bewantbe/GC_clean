...
% This version try to give positive definite covariance series
...
function R = getcovpd(X, m)

if (exist('X','var')==0 || exist('m','var')==0 || isempty(X)==1)
	disp('usage: R = getcovpd(X, m)');
...
R = zeros(p, p*(m+1));
for k = 0 : m
	R(:, 1+p*k : p+p*k) = (X(:, m+1:len) * X(:, m-k+1:len-k)') / (len-m);
end
