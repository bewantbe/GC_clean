...
% This version promised to output positive definite covariance series
...
function R = getcovpd2(X, m)

if (exist('X','var')==0 || exist('m','var')==0 || isempty(X)==1)
	disp('usage: R = getcovpd2(X, m)');
...
R = zeros(p, p*(m+1));
for k = 0 : m
	R(:, 1+p*k : p+p*k) = (X(:, k+1:len-m+k) * X(:, 1:len-m)') / (len-m);
end
